...
export function VersionCompare({ appName, currentCode, isOpen, onClose }: VersionCompareProps) {
  ...
  const loadVersions = async () => {
    try {
      ...
      console.log('[VersionCompare] Loading versions for:', appName);
      const response = await fetch(`api/versions/${appName}`);
      
      console.log('[VersionCompare] Response status:', response.status);
      
      if (!response.ok) {
        const errorData = await response.json().catch(() => ({ detail: 'Unknown error' }));
        console.error('[VersionCompare] Error response:', errorData);
        throw new Error(errorData.detail || `HTTP ${response.status}: Failed to load versions`);
      }
      
      const data = await response.json();
      console.log('[VersionCompare] Loaded', data.versions?.length || 0, 'versions');
      setVersions(data.versions || []);
    } catch (err) {
      console.error('[VersionCompare] Error:', err);
