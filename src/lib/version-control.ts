...
export async function listVersions(appName: string): Promise<VersionInfo[]> {
  console.log('[Version Control] Listing versions for:', appName);
  console.log('[Version Control] VERSIONS_DIR:', VERSIONS_DIR);
  
  try {
    await ensureVersionsDir();
    console.log('[Version Control] Versions directory ensured');
  } catch (error) {
    console.error('[Version Control] Error ensuring versions directory:', error);
    return [];
  }
  
  try {
    const entries = await fs.readdir(VERSIONS_DIR);
    console.log('[Version Control] Found', entries.length, 'files in versions directory');
    ...
    console.log('[Version Control] Found', versions.length, 'versions for', appName);
    return versions.sort((a, b) => b.version.localeCompare(a.version));
  } catch (error) {
    console.error('[Version Control] Error reading versions directory:', error);
    return [];
  }
}
