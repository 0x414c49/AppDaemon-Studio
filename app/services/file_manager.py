...
class FileManager:
    ...
    async def create_app(
        ...
        async with self._lock:
            ...
            await loop.run_in_executor(
                None,
                lambda: python_path.write_text(python_content, encoding='utf-8')
            )
            ...
            await loop.run_in_executor(
                None,
                lambda: yaml_path.write_text(yaml_content, encoding='utf-8')
            )
    ...
    def _render_template(self, name: str, class_name: str, description: str) -> str:
        ...
        return template.render(
            name=name,
            class_name=class_name,
            description=description or f"AppDaemon app: {name}"
        )
    ...
    async def read_python(self, name: str) -> str:
        ...
        return await loop.run_in_executor(
            None,
            lambda: python_path.read_text(encoding='utf-8')
        )
        ...
        async with self._lock:
            ...
            if python_path.exists():
                ...
                await self.version_control.create_version(
                    name, f"{name}.py", existing_content
                )
            ...
            await loop.run_in_executor(
                None,
                lambda: python_path.write_text(content, encoding='utf-8')
            )
    ...
    async def write_yaml(self, name: str, config: dict) -> None:
        ...
        async with self._lock:
            ...
            await loop.run_in_executor(
                None,
                lambda: yaml_path.write_text(content, encoding='utf-8')
            )
    ...
    async def delete_app(self, name: str) -> None:
        ...
        async with self._lock:
            ...
            trash_path = self.base_path / ".trash" / f"{name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            trash_path.parent.mkdir(parents=True, exist_ok=True)
            
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(
                None,
                lambda: app_path.rename(trash_path)
            )
